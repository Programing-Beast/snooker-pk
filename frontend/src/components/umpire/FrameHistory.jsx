      ...
      <div className="flex items-center gap-2 overflow-x-auto flex-1">
        {frameHistory.length > 0 ? (
          frameHistory.map((h, i) => (
            <div
              key={i}
              className="shrink-0 rounded-lg bg-panel2 border border-hairline-d px-3 py-2 min-w-[104px]"
            >
              <div className="font-display font-semibold uppercase tracking-[0.18em] text-ink-500 text-[9px]">
                Frame {i + 1}
              </div>
              <div className="font-display font-bold text-white tabular-nums text-[15px] mt-0.5">
