...
export default function MatchEndOverlay({ state }) {
  ...
  return (
    <div className="absolute inset-0 z-20 bg-night/92 backdrop-blur-sm grid place-items-center p-3 sm:p-8">
      <div className="bg-panel rounded-2xl border border-brass/50 shadow-2xl w-full max-w-2xl overflow-hidden">
        {/* Header */}
        <div className="px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-felt to-felt-900 flex items-center gap-2 sm:gap-3">
          <span className="inline-flex items-center gap-1.5 font-display font-semibold uppercase px-2.5 py-1 rounded-full leading-none tracking-[0.08em] text-[11.5px] bg-brass text-[#3a2c08] shrink-0">
            Match complete
          </span>
          <span className="text-white/80 font-display font-semibold text-[12px] sm:text-[13px] truncate">
            {state.round} · best of {state.bestOf}
          </span>
        </div>

        {/* Body */}
        <div className="p-4 sm:p-6 grid md:grid-cols-[1fr_220px] gap-4 sm:gap-6">
          <div className="text-center md:text-left">
            ...
            <div className="flex items-center gap-3 mt-2 justify-center md:justify-start">
              <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-full bg-felt grid place-items-center font-display font-extrabold text-white text-lg sm:text-xl ring-2 ring-brass shrink-0">
                {playerInitials(winner.name)}
              </div>
              <div>
                <div className="font-display font-extrabold text-white text-[20px] sm:text-[26px] leading-none">{winner.name}</div>
                <div className="flex items-center gap-1.5 text-[11px] sm:text-[12px] text-ink-400 mt-1">
                  {winner.countryCode} · {winner.tier}
                </div>
              </div>
            </div>
            <div className="font-display font-extrabold text-brass tabular-nums mt-3 sm:mt-4 text-[40px] sm:text-[60px] leading-none">
            ...
            <Link
              to="/umpire/dashboard"
              className="inline-flex items-center justify-center gap-2 font-display font-semibold rounded-md px-4 sm:px-5 py-2.5 text-[13px] sm:text-sm leading-none transition active:translate-y-px bg-felt text-white hover:bg-felt-700 mt-4 sm:mt-5"
            >
