...
export default function FrameEndOverlay({ state, dispatch }) {
  ...
  return (
    <div className="absolute inset-0 z-20 bg-night/88 backdrop-blur-sm grid place-items-center p-3 sm:p-8">
      <div className="bg-panel rounded-2xl border border-brass/40 shadow-2xl w-full max-w-xl overflow-hidden">
        {/* Header */}
        <div className="px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-felt-900 to-panel2 border-b border-hairline-d flex items-center gap-2 sm:gap-3">
          ...
          <span className="text-white font-display font-bold text-[14px] sm:text-[16px]">Frame complete</span>
        </div>

        {/* Body */}
        <div className="p-4 sm:p-6 text-center">
          <div className="font-display font-semibold uppercase tracking-[0.18em] text-ink-400 text-[11.5px]">Winner</div>
          <div className="font-display font-extrabold text-white text-[22px] sm:text-[30px] leading-none mt-1">{winner.name}</div>
          <div className="font-display font-extrabold text-brass tabular-nums mt-3 text-[36px] sm:text-[54px] leading-none">
            {winner.points} <span className="text-ink-500">–</span> {loser.points}
          </div>
          <div className="flex items-center justify-center gap-4 sm:gap-6 mt-3 sm:mt-4 text-[12px] sm:text-[13px]">
            <div>
          ...
          {/* Next breaker */}
          <div className="mt-4 sm:mt-6 pt-4 sm:pt-5 border-t border-hairline-d">
            <div className="text-ink-300 text-[12px] sm:text-[13px] mb-2.5 sm:mb-3">
