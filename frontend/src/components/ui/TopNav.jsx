...
export default function TopNav() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-night/90 backdrop-blur border-b border-white/10">
      <div className="max-w-[1200px] mx-auto px-6 h-16 flex items-center justify-between gap-4">
        ...
        <div className="ml-auto flex items-center gap-3">
          {isAuthenticated ? (
            <>
              {hasRole('admin') && (
                <Link to="/admin" className="hidden md:inline-flex btn btn-sm bg-white/10 text-white border border-white/15 hover:bg-white/20">
                  Admin
                </Link>
              )}
            ...
            <>
              <Link to="/login" className="btn btn-sm bg-white/10 text-white border border-white/15 hover:bg-white/20">Log in</Link>
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden bg-night border-t border-hairline-d px-6 py-4 space-y-2">
          {NAV_LINKS.map(link => (
            <NavLink
              key={link.to}
              to={link.to}
